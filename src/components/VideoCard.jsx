    ...
    <>
      {type === 'normal' ? (
        <div className='w-[100%] mx-auto text-white py-4 md:p-4 md:w-1/3 md:mx-0 lg:w-1/4 xl:w-1/5'>
          ...
          <Link to={`/channel/${channelId}`} className='dark:text-neutral-400 mx-2 md:mx-0 font-medium font-secondary flex items-center gap-1'>
            <span className="material-symbols-outlined text-lg flex items-center justify-center">
              check_circle
            </span>
            {channelTitle}
          </Link>
          <p className='text-neutral-500 text-sm mt-1 font-medium mx-2 md:mx-0'>{publishTime.slice(0, 10)}</p>
        </div>
      ) : (
